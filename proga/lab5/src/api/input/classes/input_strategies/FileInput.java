package api.input.classes.input_strategies;
...
import api.input.interfaces.InputStrategy;
...
public class FileInput implements InputStrategy {
    ...
    public String readNext() {
        ...
        try {
            int data;
            buffer.setLength(0); // Очищаем буфер перед использованием
            while ((data = inputStream.read()) != -1) {
                if (data == '\n' || data == '\r') {
                    break; // Чтение до конца строки
                }
